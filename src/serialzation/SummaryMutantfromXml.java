...
public class SummaryMutantfromXml extends DealXmlSax {
...
//		System.out.println(allMethod);
			...
			public void visit(Element e) {
				if (e.getName().equals("File")) {
					...
					if (s != "null") {
						...
						if (!name_opMap.containsKey("(" + name0 + "," + opType
								+ "," + op + ")")) {
							name_opMap.put("(" + name0 + "," + opType + ","
									+ op + ")", 1);
						} else {
							int count = name_opMap.get("(" + name0 + ","
									+ opType + "," + op + ")") + 1;
							name_opMap.put("(" + name0 + "," + opType + ","
									+ op + ")", count);
						}
						...
						if (!name_opTypeMap.containsKey("(" + name0 + ","
								+ opType + ")")) {
							name_opTypeMap.put(
									"(" + name0 + "," + opType + ")", 1);
						} else {
							int count = name_opTypeMap.get("(" + name0 + ","
									+ opType + ")") + 1;
							name_opTypeMap.put(
									"(" + name0 + "," + opType + ")", count);
						}
						if (!name_tm_methodMap.containsKey("(" + name0 + ","
								+ opType + "," + method + ")")) {
							name_tm_methodMap.put("(" + name0 + "," + opType
									+ "," + method + ")", 1);
						} else {
							int count = name_tm_methodMap.get("(" + name0 + ","
									+ opType + "," + method + ")") + 1;
							name_tm_methodMap.put("(" + name0 + "," + opType
									+ "," + method + ")", count);
						}
						if (!opType_opMap.containsKey("(" + opType + "," + op
							...
							int count = opType_opMap.get("(" + opType + ","
									+ op + ")") + 1;
							opType_opMap.put("(" + opType + "," + op + ")",
									count);
						}
		...
		FileList.getInstance().setJavaMList(mutantList);
	}

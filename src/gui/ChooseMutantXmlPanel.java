...
public class ChooseMutantXmlPanel extends JPanel implements ActionListener,
	...
	SummaryMutPanel summaryp = new SummaryMutPanel(Project.getInstance()
			.getJavaMutFilterLoc());
	SummaryMutPanel choosep = new SummaryMutPanel(1);
	JScrollPane jsp = new JScrollPane(showPanel);

	ChooseMutantXmlPanel() {
		getTable();
		...
		FILETableModel fTableModel = new FILETableModel(getNewTragetFiles(),
				"file");
	...
	private void getTable() {
		// out.println(xmb.getParent());
		// TODO Auto-generated method stub
		SummaryMutantfromXml gsm = new SummaryMutantfromXml();
		String[] args = new String[] {
				Project.getInstance().getXmlMutFilterLoc(), "DBHelper.java",
				"all", "classOp", "all" };
		try {
			gsm.run(args);
		} catch (Exception e) {
...
//		if(vc==null)
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == this.showChoosedMutant) {
			...
			SummaryMutPanel choosep = new SummaryMutPanel(Project.getInstance()
					.getXmlMutFilterLoc(), nameSet, opTypeSet, cmopSet,
					tmopSet, emopSet, amopSet, xmopSet);
